...
def calculated(temp_dataset, nCol, impact):
...
def minmax(temp_dataset, nCol, weights):
...
def topsis_pipy(temp_dataset, dataset, nCol, weights, impact):
    # normalizing the array
    temp_dataset = minmax(temp_dataset, nCol, weights)

    # Calculating positive and negative values
    p_sln, n_sln = calculated(temp_dataset, nCol, impact)
